...
class _AccessScreenState extends State<AccessScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_createMode) {
        final res =
            await ClinicLink.enroll(_email.text.trim(), _pass.text);
        AttributionBeacon.registration();
        final uid = res.user?.id;
        if (uid != null) await AlertRelay.bindUser(uid);
        _toast('Account created. Check your inbox to confirm.');
      } else {
        final res =
            await ClinicLink.signIn(_email.text.trim(), _pass.text);
        AttributionBeacon.login();
        final uid = res.user?.id;
        if (uid != null) await AlertRelay.bindUser(uid);
      }
      if (!mounted) return;
  ...
  Future<void> _forgotPassword() async {
    ...
    try {
      await ClinicLink.resetPassword(email);
              ...
              children: [
                ...
                Container(
                  ...
                  child: const Icon(Icons.medical_services_rounded,
                      color: ClinicPalette.tealDeep, size: 28),
                ),
                const SizedBox(height: 14),
                Text(_createMode ? 'Create your record' : 'Welcome back',
                    style: ClinicType.title()),
                const SizedBox(height: 8),
                Text(
                  'An account syncs your flock records and vaccination reminders across devices. It is optional — every tool works fully offline.',
